...
int GetLeftIndexFromId(int id) {
  return id * 2 - 1;
}

int GetRightIndexFromId(int id) {
  return id * 2;
}
...
int main() {
  ...
  while (true) {
    ...
    for (int i = 0; i < vertex_count; i++) {
      ...
      int edges = tmp.size() / 2;
      for (int j = 0; j < edges; j++) {
        solver.AddEdge(GetLeftIndexFromId(i + 1),
                       GetRightIndexFromId(tmp[j * 2]),
                       1,
                       tmp[j * 2 + 1]);
      }
    }
    ...
    for (int i = 1; i <= vertex_count; i++) {
      solver.AddEdge(GetSrcIndex(), GetLeftIndexFromId(i), 1, 0);
      solver.AddEdge(GetRightIndexFromId(i), GetTargetIndex(), 1, 0);
    }

    long long cost;
    int flow = solver.MinCostMaxFlow(GetSrcIndex(), GetTargetIndex(), cost);
    if (flow < vertex_count) {
      cout << "N" << endl;
    } else {
      cout << cost << endl;
    }
