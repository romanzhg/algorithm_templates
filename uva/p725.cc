...
int convertToInt(const vector<int>& permutation) {
	int rtn = 0;
	for (int i = 0; i < 5; i++) {
		rtn = rtn * 10 + permutation[i];
	}
	return rtn;
}

bool hasConflict(int v, int o) {
	bool used[10];
...
int main() {
	int N;
	vector<vector<int>> permutations;
	genPermutationsK(permutations, 10, 5);
	...
	while (scanf("%d", &N) == 1 && N != 0) {
		...
		for (const vector<int>& permutation : permutations) {
			int v = convertToInt(permutation);
			int o = v * N;
			if (o > 99999) {
				continue;
			}
