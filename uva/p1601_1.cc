...
// 2. State compression.
// 3. Use array to implement a queue. To have this optimization, a total
// number of possible states is estimated(180 ^ 3).
// However is seems this optimization made the runtime worse.
// The problem may be having such a big array (24MB) in the memory
// harms the cache performance.
// Simply use the STL queue performs better.
#include <cfloat>
#include <climits>
#include <cmath>
#include <cstdio>
...
void expand(const State& curState, State& newState, int count) {
	if (count == 0) {
		...
		if (visited.find(newEncoded) != visited.end()) {
			return;
		}
		visited.insert(newEncoded);
		states[endIndex++] = newEncoded;
		// A boundary check, could make startIndex and endIndex circular.
		// Since the in queue states won't be that many.
		if (endIndex > (MAX_STATES - 1000)) {
			exit(-1);
		}
		return;
	}
...
void solve(const State& initial, const State& target, int count) {
	...
	startIndex = 1;
	endIndex = 1;
	states[endIndex++] = encode(initial);
	// Note: if visited is checked when adding new nodes, the visited status
	// of the initial node needs to be set up front.
	// If visited is checked after polling from the queue, we don't need to
	// set visited status of the initial node here.
	visited.insert(encode(initial));

	int levelSize = 1;
	while (startIndex < endIndex) {
		unsigned int curEncoded = states[startIndex++];
		levelSize--;
